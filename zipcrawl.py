...
def stdev(thelist):
    ...
    thelist2=[]
    for i in thelist:
        thelist2.append((i-ave)*(i-ave))
    if len(thelist)>1:
        return (sum(thelist2)/(len(thelist)-1))**.5
    else:
        return 0
...
ziplist=[]
h=open("boston_zips.csv",'r')
for line in h:
    ziplist.append(str(line.split(',')[0]))
###ziplist.append("19143")
...
#priceregex= re.compile('result-meta[\'|"]><span\sclass=[\'|"]result-price[\'|"]>\$(\d*?)</span>')
priceregex= re.compile('result-meta[\'|"]>\n\s*<span\sclass=[\'|"]result-price[\'|"]>\$(\d*?)</span>')
allbands=[]
f = open("zip_parkingprice_bos.txt",'w')#individual prices
g = open("zip_price_raw_csvbos_stdev.csv",'w')#averages
#G=nx.DiGraph()
avelist=[]
#while len(crawled)>-1:
cutoff=500
for zip0 in ziplist:
    #https://boston.craigslist.org/search/prk?search_distance=0&postal=02135&availabilityMode=0
    #https://philadelphia.craigslist.org/search/prk?search_distance=0&postal=19139&availabilityMode=0
    urlname="https://boston.craigslist.org/search/prk?search_distance=0&postal="+zip0+"&availabilityMode=0"
    try:
 ...
 #   startPos = msg.find('<body')
    ...
    for i in range(3):
        for eachprice in prices:
            ...
            if float(eachprice)>cutoff or len(eachprice)<2:
    ...
    if len(pricelist)!=0:
        avelist.append([(zip0),sum(pricelist)/len(pricelist),stdev(pricelist)])
#        f.write("Average for" + zip0+": " + str(sum(pricelist)/len(pricelist)))
g.write("Zip code, Price($), stdev($)"+'\n')
for mytuple in avelist:
    g.write(mytuple[0]+',' + str(mytuple[1])+',' + str(mytuple[2]) + '\n')
